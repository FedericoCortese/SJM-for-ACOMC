df100000045=read.table("100000045.txt",header = T)

# Not sure which window to select (too many time jumps)

start=which.min(abs(df100000045$t-9208945.50 ))
end=which.min(abs(df100000045$t-233093568.40))

df100000045=df100000045[start:end,]

summary(diff(df100000045$t))
plot(diff(df100000045$t),type='p')
...
data2=merge(data_fin,df100000045,by="t")
data2 <- data2 %>%
...
p_I <- ggplot(data2) + 
  labs(title = "100000045", x = "t", y = "Theta") +
...
sat_mod=SJM_sat(Y)
Lnsat=sat_mod$Lnsat
start_100000045=Sys.time()
est100000045 <- parallel::mclapply(1:nrow(hp),
                                   function(x)
...
end_100000045=Sys.time()
elapsed_100000045=end_100000045-start_100000045
save(data_fin,Y,est100000045,file="100000045.RData")

modsel100000045=data.frame(hp,
                           FTIC=unlist(lapply(est100000045,function(x)x$FTIC))
)

best_mod=modsel100000045[which.min(modsel100000045$FTIC),]
best_mod

sel=100
#sel=68
estw100000045=data.frame(var=colnames(Y),
                         weight=est100000045[[sel]]$est_weights)

estw100000045=estw100000045[order(estw100000045$weight,decreasing = T),]
head(estw100000045,15)

# Merge with a theta and t for plotting
data_a_theta=tail(data[,c("t","a","theta")],dim(Y)[1])

df_res_100000045=data.frame(data_a_theta,
                            Y,
                            State=est100000045[[sel]]$est_states
)

df_res_100000045 <- df_res_100000045 %>%
...
df_segments_a <- df_res_100000045 %>%
  group_by(Segment) %>%
...
p_a_res <- ggplot(data = df_segments_a) + 
  ...
  scale_color_manual(values = 1:max(df_res_100000045$State)) +
  labs(title = "100000045", 
       x = "Time (t)", 
       y = "Values (a)") +
  theme_minimal()
ggplotly(p_a_res)

df_segments_theta <- df_res_100000045 %>%
  group_by(Segment) %>%
...
# Check feat by feat
pp <- ggplot(data=df_res_100000045,aes(x=t)) + 
