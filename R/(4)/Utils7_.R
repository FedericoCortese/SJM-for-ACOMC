...
angular_distance <- function(x, y) {
  # Convertire gli angoli in radianti, se sono in gradi
  # x_rad <- x * pi / 180
  # y_rad <- y * pi / 180
  ...
  # Trasformare gli angoli nel piano complesso
  z_x <- complex(real = cos(x_rad), imaginary = sin(x_rad))
  z_y <- complex(real = cos(y_rad), imaginary = sin(y_rad))
...
circular_distance <- function(x, y) {
  diff <- abs(x - y) # Differenza assoluta
  dist <- pmin(diff, 2 * pi - diff) # Distanza circolare
  return(dist)
}
